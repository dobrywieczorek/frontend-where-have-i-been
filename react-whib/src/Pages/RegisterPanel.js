...
export default function RegisterPanel() {
    ...
    async function Register(event) {
        ...
        if (name.includes('#')) {
            setError(t('cantcontainhash'));
            return;
        }
        
        if (!validateEmail(email)) {
            setError(t('wrongemailerror'));
            return;
        }
        ...
        <>
            <div className="mx-auto h-screen grid content-center">
                <div className="flex flex-col items-center pb-28">
                    <h3 className="text-2xl font-bold mb-2">{t('registertitle')}</h3>
                    <p className="mb-10">{t('registernewaccount')}</p>
                    ...
                    <p className="mt-3">{t('alreadyhaveanaccount')} <Link className="text-blue-400 hover:text-blue-700" to="/login">{t('logintoyouraccount')}</Link></p>
                </div>
            </div>
        </>
    )
